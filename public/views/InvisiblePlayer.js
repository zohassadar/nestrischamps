...
export default class InvisiblePlayer extends Player {
	constructor(...args) {
		super(...args);
	...
	startClearAnimation(remaining_frames, board) {
	...
	setFrame(frame) {
		...
		if (this.last_field_str === field_str) {
			...
			if (this.clear_animation_remaining_frames-- > 0) {
				...
				this.startClearAnimation(10, new_board);
				...
				this.startClearAnimation(CLEAR_ANIMATION_FRAMES - 1, new_board);
				...
				// preparing clear animation
				this.startClearAnimation(CLEAR_ANIMATION_FRAMES - 2, new_board);
		...
		if (this.invisible_frame_buffer.length >= 3) {
			super.setFrame(this.invisible_frame_buffer.shift());
		}
	}
}
